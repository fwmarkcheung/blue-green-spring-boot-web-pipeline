...
The build template contains a default source code repo for a java application compatible with this pipelines architecture (https://github.com/fwmarkcheung/blue-green-spring-boot-web-pipeline.git).
...
* Two openjdk8 DeploymentConfig's: blue one and green one
* Two Service definition's
* 3 Routes -blue, green, and the current route
...
For convenience, this pipeline script is already included in the following git repository, based on a [Simple Spring Boot Web app](https://github.com/fwmarkcheung/blue-green-spring-boot-web-pipeline.git) app.  The app displays a message that will change color based on which deployment is live in the production project.

https://github.com/fwmarkcheung/blue-green-spring-boot-web-pipeline.git
...
oc process openshift//jenkins-ephemeral | oc apply -f- -n spring-boot-web-build
...
oc process -f openshift/deployment/template.yml -p APPLICATION_NAME=spring-boot-web -p NAMESPACE=spring-boot-web-dev  | oc apply -f-

oc process -f openshift/deployment/template.yml -p APPLICATION_NAME=spring-boot-web -p NAMESPACE=spring-boot-web-stage  | oc apply -f-
...
oc process -f openshift/deployment/template-bg.yml -p APPLICATION_NAME=spring-boot-web -p NAMESPACE=spring-boot-web-prod  | oc apply -f-
...
oc process -f openshift/builds/template.yml -p APPLICATION_NAME=spring-boot-web -p NAMESPACE=spring-boot-web-build -p IMAGE_STREAM_TAG_NAME=redhat-openjdk18-openshift:1.1 | oc apply -f-
